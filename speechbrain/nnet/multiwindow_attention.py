"""Library implementing multi-window multi-head attention.
...
def window_reverse1d(windows, window_size, W: int, attention_padding=0):
    ...
    if B == 0:
        ...
        B = 1
    x = windows.reshape(B, (W + attention_padding) // window_size, window_size, -1)
...
class WindowedAttentionHead(nn.Module):
    ...
    def forward(self, q, k, v, key_padding_mask=None, attn_fill_value=-65000):
        ...
        q = window_partition1d(q, self.window_size, attention_padding=window_attn_padding)
        k = window_partition1d(k, self.window_size, attention_padding=window_attn_padding)
        v = window_partition1d(v, self.window_size, attention_padding=window_attn_padding)
        ...
        if key_padding_mask is not None:
            ...
            key_padding_mask_windowed = window_partition1dkey(key_padding_mask, self.window_size, attention_padding=window_attn_padding).bool()
        ...
        shifted_x = window_reverse1d(x, self.window_size, W=W, attention_padding=window_attn_padding)
        if self.shift_size > 0:
...
class MultiWindowMultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        # print("in MultiWindowMultiheadAttention forward")
